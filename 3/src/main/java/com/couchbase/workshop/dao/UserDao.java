...
public class UserDao extends AJsonSerializable implements IAsyncDao {
    ...
    public Observable<User> get() {
        ...
        //If an error occours it will be passed to the mapped Observable by 
        //allowing to handle it in the final subscribe method
    ...
    protected JsonDocument toJson(Object obj) {
        ...
        if (tmpUser.getUid() != null) json.put(PROP_UID, tmpUser.getUid());
        if (tmpUser.getFirstName() != null ) json.put(PROP_FIRSTNAME, tmpUser.getFirstName());
        if (tmpUser.getLastName() != null ) json.put(PROP_LASTNAME, tmpUser.getLastName());
        if (tmpUser.getEmail() != null ) json.put(PROP_EMAIL, tmpUser.getEmail());
        if (tmpUser.getBirthDay() != null ) json.put(PROP_BDAY, tmpUser.getBirthDay().getTime());
